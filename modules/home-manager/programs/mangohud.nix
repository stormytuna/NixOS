{
  ...
  options.modules.programs.mangohud = {
    enable = lib.mkEnableOption "Vulkan and OpenGL overlay for monitoring FPS, CPU/GPU load and much more";
  };

  config = lib.mkIf config.modules.programs.mangohud.enable {
    programs.mangohud = {
      enable = true;
